...
<form method="post" enctype="multipart/form-data" ref="form">
    ...
    <button type="submit" class="btn btn-primary"
            @click.prevent="upload" :disabled="isUploading">
    <span class="spinner-border spinner-border-sm me-2"
          role="status" aria-hidden="true"
          v-if="isUploading"></span>
    {{ isUploading ? '上傳中，請稍候' : '確認上傳' }}</button>
</form>

<template v-if="clipboardVisible">
...
export default {
    ...
    props: ['clipboardVisible', 'uploadState'],
